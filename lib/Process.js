	...
	async function checkProfit(pairsToTest, markets) {
		...
		for (let j = 0; j < pairsToTest.length; j += 1) {
			...
			if (!fail) {
				...
				if (loops === 0) {
					if (profitTable > 0.0200) {
						loops = 1
						var orderIdnew = 0;	
						console.log(chain)					
						var ascOrder = percen.sort((a, b) => a > b ? 1 : -1);
						console.log('Profit : ' + profitTable + '%' + ' Fee : ' + fee.toFixed(4) + ' Amount : ' + percen[0] + "%")
						let failcheck = false;
						var budgets = BUDGET
						...
						for (let i = 0; i < 3; i += 1) {
							...
							if (i === 1){
								binance.bookTickers(orders[2].market.replace('/', ''), (error, ticker) => {									
									var bookTickerss = (amount*parseFloat(ticker.bidPrice))
									var parcenbookTicker = bookTickerss*0.001
									bookTickerss = bookTickerss-parcenbookTicker
									if (bookTickerss < parseFloat(amountstart)){
										i = 8
										waitone = 8
										console.log("\t ->(Price Low | " + ticker.bidPrice + " | " + bookTickerss.toFixed(4) + "/" + amountstart +")")
									} else {
										console.log("\t ->(Price Correct | " + ticker.bidPrice + " | " + bookTickerss.toFixed(4) + "/" + amountstart +")")
										waitone = 8
									}
							...
							if (i < 5 ){
								if (i === 2){
									binance.bookTickers(orders[i].market.replace('/', ''), (error, ticker) => {									
										if ((amount*parseFloat(ticker.bidPrice)) > parseFloat(budgets)){
											price = ticker.bidPrice;
											console.log("\t ->(UPDATE - Price 1 | " + price + ")")
											waittwo = 8
										} else {
											if ((amount*parseFloat(ticker.askPrice)) > parseFloat(budgets)){
