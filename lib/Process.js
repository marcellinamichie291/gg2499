				...
				if (profitTable > 0.0100) {
					if (loops === 0) {
						...
						for (let i = 0; i < 3; i += 1) {
							...
							var prnew = 0
							if (cutStatus){
								var cancelOrder = binance.cancelOrders(orders[i].market.replace('/', ''), (error, response, symbol) => {
									if (response.status === 'CANCELED') {
										amnew = response.origQty
										prnew = response.price
										cutcheck = true
									}
								})
								while (true) {
								...
								let balances_ = amnew*prnew
								var _exchange = exchangeInfo.symbols.find(el => el.symbol === orders[0].market.replace('/', ''));
								if (_exchange.filters[2].minQty === '1.00000000') {
									balances_ = Math.floor(balances_)
								} else {
									balances_ = getValidQuantity(balances_, _exchange.filters[2].minQty)
								}
								if (orders[i].side === 'sell'){
									binance.marketBuy(orders[0].market.replace('/', ''), balances_);
								} else {
									binance.marketSell(orders[0].market.replace('/', ''), balances_);
								}
							}*/
						}						
