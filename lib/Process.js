				...
				if (loops === 0) {					
					...
					if (amuntcheck > 0.0100) {
						...
						while (true) {				
							...
							var waitone = undefined
							var amountnew = amount-percensx
							binance.bookTickers(orders[2].market.replace('/', ''), (error, ticker) => {									
								var bookTickerss = (amountnew*parseFloat(ticker.bidPrice))
								var bookTickersx = (amountnew*parseFloat(ticker.askPrice))
								if (bookTickerss > parseFloat(budgetsx)){
									newprice = ticker.bidPrice
									waitone = 2							
								} else {
									if (bookTickersx > parseFloat(budgetsx)){
										newprice = ticker.bidPrice
										waitone = 2	
									} else {
										waitone = 9									
										console.log("\t ->(Price Low | " + ticker.bidPrice + "/" + (orders[2].price).toFixed(8) + " | " + bookTickerss.toFixed(4) + "/" + budgetsx.toFixed(4) +")")
									}	
								}								
							});							
							while (true) {
