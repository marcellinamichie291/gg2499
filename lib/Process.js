...
module.exports = async function (BASE_USD_BUDGET = BUDGET, FEE = FEERate) {
	...
	binance.allOrders("BUSDUSDT", (error, orders, symbol) => {
	  console.info(symbol+" orders:", orders);
	});
	
	console.log( await binance.getOrder({
	    symbol: 'BUSDUSDT',
	    orderId: 726635942,
	  }),
	)
					...
					if (profitTable > 0.0010) {
						...
						for (let i = 0; i < 3; i += 1) {
							...
							/*if (orderIdnew != 0){
								let ckStatus = false
								while (true) {
									binance.orderStatus(orders[i].market.replace('/', ''), orderIdnew, (error, orderStatus, symbol) => {
									...
									if(ckStatus) {
										console.log("\t ->(" + orderIdnew + ") Filled (" + orders[i].market + ")" + " Amount " + parseFloat(amount).toFixed(8) + "@" + parseFloat(price).toFixed(8))
										break
									} else {
										await sleep(300);
									}							
