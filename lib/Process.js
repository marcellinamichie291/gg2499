		...
		for (let j = 0; j < pairsToTest.length; j += 1) {
			...
			if (!fail) {
				...
				if (loops === 0) {					
					...
					var percensx = amuntcheck*0.0015
					amuntcheck = (amuntcheck-percensx)*(orders[2].price).toFixed(8)				
					if (amuntcheck > parseFloat(BUDGET)) {
						loops = 1						
						console.log(chain)					
						...
						while (true) {
							...
							var pricenew = (orders[2].price).toFixed(8);
							...
							binance.bookTickers(orders[2].market.replace('/', ''), (error, ticker) => {									
								...
								if (bookTickerss < parseFloat(amountstart)){
									...
									pricenew = parseFloat(ticker.bidPrice)
									waitone = 0
								}							
							});
