		...
		for (let j = 0; j < pairsToTest.length; j += 1) {
			...
			if (!fail) {
				...
				if (loops === 0) {					
					...
					var exchangeInfo_ = exchangeInfo.symbols.find(el => el.symbol === orders[2].market.replace('/', ''));
					if (exchangeInfo_.filters[2].minQty === '1.00000000') {
						amuntcheck = Math.floor(amuntcheck)
					} else {
						amuntcheck = getValidQuantity(amuntcheck, exchangeInfo_.filters[2].minQty)
					}
					amuntcheck = amuntcheck*(orders[2].price).toFixed(8)
					amuntcheck = amuntcheck - parseFloat(BUDGET)
					if (amuntcheck > 0.0010) {
						loops = 1						
						console.log(chain)					
						...
						while (true) {
							...
							if (parseFloat(_balances).toFixed(8) < (parseFloat(budgets)-1)){
								...
								amount = parseFloat(_balances).toFixed(8);
							...
							if (waitone < 5){
								...
								console.log("\t ->(OrderID | " + orderIdnew + " | " + orders[1].market.replace('/', '') + " | " + price + ")")
								...
								if (orderIdnew === undefined){
									...
									if (cancelOrder === undefined){
										...
										console.log("\t ->(OrderID | " + orderIdnew + " | " + chain[2]+chain[3] + " | " + price + ")")
									}
								} else {
									console.log("\t ->(OrderID | " + orderIdnew + " | " + chain[2]+chain[3] + " | " + price + ")")
								}
