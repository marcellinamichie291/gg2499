		...
		for (let j = 0; j < pairsToTest.length; j += 1) {
			...
			if (!fail) {
				...
				if (loops === 0) {					
					...
					var newpricx = exchangeInfo.symbols.find(el => el.symbol === marketold);
					var newsize = (orders[2].price.toFixed(8)-parseFloat(newpricx.filters[0].tickSize))		
					amuntcheck = amuntcheck*newsize
					amuntcheck = amuntcheck-budgets
				
					if (amuntcheck > 0.0050) {
						loops = 1						
						console.log(chain)	
