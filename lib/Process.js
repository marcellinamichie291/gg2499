						...
						for (let i = 0; i < 3; i += 1) {
							...
							if (i === 1){					
								...
								binance.bookTickers(orders[2].market.replace('/', ''), (error, ticker) => {
									bidPrices = ticker.bidPrice;
									
									var amountddd = ((orders[2].amount).toFixed(8))*0.001
									var amountnew = ((orders[2].amount).toFixed(8)) - amountddd						
									var valueamount = amountnew.toFixed(8)*parseFloat(bidPrices)
									if (valueamount > parseFloat(BUDGET)) {
										console.log(valueamount + " | " + bidPrices + " | " + valueamount)
										amountupdate = bidPrices;
									} else {
