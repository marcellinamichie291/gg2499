						...
						for (let i = 0; i < 3; i += 1) {
							...
							if (i < 5 ){
								...
								if (orders[i].side === 'buy') {
									...
									binance.buy(orders[i].market.replace('/', ''), parseFloat(amount), parseFloat(price), { type: 'LIMIT' },async (error, response) => {																
									...
									waitOrder = orders[i].market.replace('/', '')
									binance.sell(orders[i].market.replace('/', ''), parseFloat(amount), parseFloat(price), { type: 'LIMIT' },async (error, response) => {
										orderIdnew = response.orderId;
										waitOrder = undefined
								...
								if (orderIdnew != 0 && i < 5){									
									...
									while (true) {
										var orderStatus = 0;
										binance.allOrders(marketold,async (error, orders, symbol) => {																	
											try {										
