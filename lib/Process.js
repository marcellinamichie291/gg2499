...
module.exports = async function (BASE_USD_BUDGET = BUDGET, FEE = FEERate) {
										...
										if(ckStatus) {
											...
											if (i === 2){
												...
												if (cutloss){
													...
													binance.marketSell(marketold, parseFloat(amount), (error, response) => {
	...
	/*binance.marketSell(orders[i].market.replace('/', ''), parseFloat(amount), (error, response) => {
		console.info("Market Sell response", response);
		console.info("order id: " + response.orderId);
	});*/
	
	//binance.marketSell(orders[i].market.replace('/', ''), parseFloat(amount));
